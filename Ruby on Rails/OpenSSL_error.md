...
- rbenvで他のバージョンのRubyをインストールしてバージョンを変更することの検証はできていない。ここでbuild failedのエラーが出る可能性大
- rails のバージョンが意図せず6.1.5になってしまった。既存のアプリケーションでの作業に支障が出る可能性あり。
	- 既存のプロジェクトのディレクトリに入って`bin/rails s`すると、bundlerのバージョンが合ってないよみたいなエラーが出たので、ここはまたあとで試してみる。
- 諸々課題は残っているだろうから、ちょくちょく気になった点をこちらのドキュメントに追加していく。

新環境でRailsの環境構築している。bundle execしなくても、既存のRails立ち上げはできたけど、やっぱりRails newするとOpenSSLのエラーがでる。

[こちら](https://gist.github.com/otavioschwanck/b16f7753d5e075eff94300cf37e85292) 参考になるだろうか。後で調査。
